...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    axios.get(`/v1/products/${id}`).then(r => setProduct(r.data)).catch(() => navigate('/'))
  }, [id])

  const addToCart = async () => {
    await axios.post(`/v1/cart/${sessionId}/items`, { product_id: id, qty })
    setAdded(true)
    setTimeout(() => setAdded(false), 2000)
  }

  if (!product) return <div className="flex justify-center items-center h-64"><div className="animate-spin w-8 h-8 border-4 border-indigo-600 border-t-transparent rounded-full"/></div>
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row gap-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mb-2">⭐ {product.rating_rate} ({product.rating_count} reviews)</p>
